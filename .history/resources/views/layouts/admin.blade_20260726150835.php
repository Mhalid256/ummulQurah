                    ...
                    <div class="navbar-nav-right d-flex align-items-center" id="navbar-collapse">
                        <div class="navbar-nav align-items-center">
                            @if(auth()->user()->isSuperAdmin())
                                <form method="POST" action="{{ route('admin.switch-organization') }}" class="d-flex align-items-center">
                                    @csrf
                                    <label class="me-2 mb-0 small text-muted">Acting as:</label>
                                    <select name="organization_id" class="form-select form-select-sm" style="width:auto;" onchange="this.form.submit()">
                                        <option value="">— All Organizations (platform view) —</option>
                                        @foreach(\App\Models\Organization::orderBy('name')->get() as $org)
                                            <option value="{{ $org->id }}" @selected(session('acting_organization_id') == $org->id)>{{ $org->name }}</option>
                                        @endforeach
                                    </select>
                                </form>
                            @else
